...
export default function TemplateSubmissions() {
  ...
  const filteredSubmissions = submissions.filter(s => {
    ...
    const numericalId = s.numericalId?.toString() || "";
    const formNumericalId = s.formNumericalId?.toString() || "";
    const id = s._id || "";
    const searchLower = searchTerm.toLowerCase();
    return submitter.toLowerCase().includes(searchLower) || 
           numericalId.includes(searchLower) ||
           formNumericalId.includes(searchLower) ||
      ...
      <div className="space-y-4">
        <div className="flex flex-col md:flex-row items-start md:items-center gap-3">
          <div className="flex-1 w-full bg-white p-3 rounded-lg border border-gray-200 flex items-center gap-2">
            ...
            <input
              type="text"
              placeholder="Search by submitter, Form ID, or Submission ID..."
          ...
          {loading ? (
            <table className="w-full text-left">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                    <th className="px-6 py-3 text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Submission ID</th>
                    <th className="px-5 py-3 text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Form ID</th>
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="text-sm font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors font-mono">
                              {sub.numericalId ? `#${sub.numericalId.toString().padStart(4, '0')}` : sub._id.slice(-8).toUpperCase()}
                            </span>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-4">
                        <span 
                          className="text-sm font-mono text-gray-700 cursor-help"
                          title={`Raw ID: ${sub.templateId?.formId || sub.templateId?._id || sub.formId?._id || sub._id || 'N/A'}${sub.formNumericalId || sub.templateId?.numericalId || sub.formId?.numericalId ? ` | Display: F-${(sub.formNumericalId || sub.templateId?.numericalId || sub.formId?.numericalId).toString().padStart(3, '0')}` : ''}`}
                        >
                          {sub.formNumericalId ? `F-${sub.formNumericalId.toString().padStart(3, '0')}` : (sub.templateId?.numericalId ? `F-${sub.templateId.numericalId.toString().padStart(3, '0')}` : (sub.formId?.numericalId ? `F-${sub.formId.numericalId.toString().padStart(3, '0')}` : (sub.templateId?.formId || sub.templateId?._id || sub.formId?._id || "—")))}
                        </span>
                      </td>
                      <td className="px-5 py-4">
