...
export default function FormsList() {
  ...
  const [activeTab, setActiveTab] = useState("ACTIVE"); // ACTIVE, DRAFT, ARCHIVED, or TEMPLATES
  ...
  const getFilteredItems = () => {
    if (activeTab === "TEMPLATES") {
      ...
      // For regular forms AND published templates (templates should appear in Active tab)
      const formsToShow = activeTab === "ACTIVE" 
        ? [...regularForms, ...templateForms.filter(t => t.status === "PUBLISHED")]
        : regularForms;
      
      return formsToShow.filter(f => {
        const matchesSearch = f.formName.toLowerCase().includes(searchTerm.toLowerCase());
        const matchesTab = 
          activeTab === "ARCHIVED" ? f.status === "ARCHIVED" :
          activeTab === "DRAFT" ? f.status === "DRAFT" :
          activeTab === "ACTIVE" ? (f.status === "PUBLISHED" || (f.isTemplate && f.status === "PUBLISHED")) :
          true;
  ...
  return (
    <div className="space-y-3 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Form Creation</h1>
          <p className="text-xs text-gray-500">Create and manage your forms and templates.</p>
        </div>
        {user?.role === "PLANT_ADMIN" && (
          ...
          <div className="flex items-center gap-1 bg-slate-100/60 backdrop-blur-sm p-1.5 rounded-xl w-fit border border-slate-200/60 shadow-sm">
            <button
              onClick={() => setActiveTab("ACTIVE")}
              className={`px-4 py-1.5 text-sm font-semibold rounded-lg transition-all ${
                activeTab === "ACTIVE"
                  ? "bg-gradient-to-r from-indigo-50 to-purple-50 text-indigo-700 shadow-md shadow-indigo-100/50"
        ...
        <input
          type="text"
          placeholder={activeTab === "TEMPLATES" ? "Search templates..." : "Search forms..."}
                ...
                <thead className="bg-gray-50 border-b border-gray-100">
                  <tr>
                    ...
                    <th className="px-4 py-2.5 text-xs font-bold text-gray-500 uppercase tracking-wider">
                      {activeTab === "TEMPLATES" ? "Template Name" : "Form Name"}
                    </th>
                    {activeTab !== "TEMPLATES" && (
                      <th className="px-4 py-2.5 text-xs font-bold text-gray-500 uppercase tracking-wider">
                        Form ID
                  ...
                  {filteredItems.map((item) => {
                    ...
                    return (
                      <tr 
                        ...
                        onClick={() => {
                          if (activeTab === "ACTIVE" && !item.isTemplate && !isArchived) {
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <div className={`p-1.5 rounded-md ${isArchived ? 'bg-gray-100 text-gray-500' : activeTab === "TEMPLATES" ? 'bg-purple-50 text-purple-600' : 'bg-indigo-50 text-indigo-600'}`}>
          ...
          {filteredItems.length === 0 && (
            <div className="text-center py-12 bg-gray-50/30">
              ...
              <h3 className="text-gray-900 font-medium text-sm">
                {activeTab === "TEMPLATES" ? "No templates found" : "No forms found"}
              </h3>
              <p className="text-gray-500 text-xs">
                {activeTab === "TEMPLATES" ? "Create a new template to get started." : "Create a new form to get started."}
              </p>
            </div>
          )}
        </div>
