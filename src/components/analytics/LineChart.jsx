...
export default function LineChart({ data, title, xLabel = "Date", yLabel = "Count" }) {
  if (!data || !Array.isArray(data) || data.length === 0) {
    return (
      <div className="bg-white rounded-3xl shadow-sm p-8 border border-gray-100 h-full flex flex-col min-h-[400px]">
  ...
  return (
    <div className="bg-white rounded-3xl shadow-sm p-8 border border-gray-100 h-full flex flex-col min-h-[400px] transition-all hover:shadow-md">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h3 className="text-xl font-bold text-gray-900 tracking-tight">{title}</h3>
          <p className="text-xs text-gray-400 font-semibold mt-1 uppercase tracking-wider">Activity Volume Over Time</p>
        </div>
      </div>
