...
export default function ModernFormBuilder({ formId, isTemplate: isTemplateProp }) {
  const { user } = useAuth();
  const navigate = useNavigate();
  const { view: activeViewFromParams } = useParams();
  const [searchParams] = useSearchParams();
  
  const isTemplate = isTemplateProp ?? (user?.isTemplateManagementEnabled ?? true);
    ...
    return () => {
      if (stateRef.current.isDirty && !saveInProgress.current) {
        ...
        if (formId && formId !== "new" && formId !== "create") {
          formApi.updateForm(formId, payload).catch(e => console.error("Unmount save failed", e));
        } else {
          payload.formId = formName.toLowerCase().replace(/\s+/g, '-') + '-' + Date.now().toString(36);
          formApi.createForm(payload).catch(e => console.error("Unmount create failed", e));
        }
      }
    };
      ...
      if (response && response.data) {
        ...
        if (form.sections && form.sections.length > 0) {
          setSections(form.sections.map(s => ({
            ...s,
            id: s.sectionId || s.id || `section-${Math.random().toString(36).substr(2, 9)}`,
            fields: (s.fields || []).map(f => ({
              ...f,
              id: f.fieldId || f.id || `field-${Math.random().toString(36).substr(2, 9)}`
            }))
          })));
        }
  ...
  const handleSave = useCallback(async (isPublish = false, silent = false) => {
    ...
    try {
      ...
      if (!silent) toast.error(err.response?.data?.message || "Error saving form");
      setSaveStatus("Error");
    } finally {
